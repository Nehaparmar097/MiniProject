     ...
     //  [Authorize(Roles = "Donor,Admin")]
      ...
      //  [Authorize]
       ...
       // [Authorize]
        ...
        [HttpGet("GetAllDonorByUserId/{userId}")]
        public async Task<ActionResult<IEnumerable<BloodStockResponseDTO>>> GetAllDonorByUserId(int userId)
        {
            try
            {
                var bloodStocks = await _service.GetAllDonorByUserId(userId);
        ...
        [HttpDelete("dlete/{id}")]
        public async Task<IActionResult> DeleteBloodStockById(int id)
        {
            try
            {
                var result = await _service.DeleteBloodStockByIdAsync(id);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPut("update/{id}")]
        public async Task<IActionResult> UpdateBloodStock(int id, BloodStockDTO bloodStockDto)
        {
            try
            {
                var result = await _service.UpdateBloodStockAsync(id, bloodStockDto);
                return Ok(result);
            }
            catch (BloodStockNotFoundException)
            {
                return NotFound();
            }
        ...
        [HttpGet("countAvailableByType")]
        public async Task<IActionResult> CountAvailableBloodByTypeAsync()
        {
            try
            {
                var bloodCounts = await _service.CountAvailableBloodByTypeAsync();
                return Ok(bloodCounts);
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Internal server error: {ex.Message}");
